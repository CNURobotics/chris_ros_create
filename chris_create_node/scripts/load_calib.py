...
package_name = 'chris_create_node'

import roslib
roslib.load_manifest(package_name)
import rospy
from create_node.srv import SetCreateMode
import yaml
import os
import rospkg
...
service = 'create_node/set_operation_mode'
...
if __name__ == '__main__':
    ...
    if docs is None:
         ...
         whichfile2 = os.path.expanduser(ros_home +"/chris_create/" +str(kinect_serial) + ".yaml")
    ...
    if docs is None:
        rospy.logwarn("Could not load a calibration file for this create, it is recommended to run create_calibration to generate a new file.")
    ...
    for doc in docs:
        client = dynamic_reconfigure.client.Client("create_node")

        resp = client.update_configuration(doc)

    # we're done
    rospy.loginfo("Create calibration successfully loaded from %s", whichfile)
